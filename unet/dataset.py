...
class ImageToImage2D(Dataset):
    ...
    def __init__(self, dataset_path: str, joint_transform: Callable = None, one_hot_mask: int = False) -> None:
        ...
        masks_list = os.listdir(self.output_path)
        ...
        for image_filename in images_list:
            image_filename_noext = image_filename.split('.')[0]
            mask_filenames = [msk for msk in masks_list if image_filename_noext == split(['.'],msk)[0]]
            ...
            if len(mask_filenames) == 1:
                self.images_list.append(image_filename)
                self.masks_list.append(mask_filenames[0])
            else:
                a=0
        self.dimensions = [d1, d2, d3]
        if joint_transform:
    ...
    def __getitem__(self, idx):
        ...
        mask = io.imread(os.path.join(self.output_path, mask_filename), as_gray=False)
